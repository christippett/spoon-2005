...
// SPOON PLANS - <spoon_plans>
ob_start();

$result = $db->query('SELECT * FROM plans ORDER BY \'id\' ASC') or error('Unable to modify user info', __FILE__, __LINE__, $db->error());
?>
<div id="plans">
  <a href="/plans.php"><img src="<?=$pun_config['o_base_url']?>/images/front_plans.gif" alt="Plans" width="89" height="43" /></a><br />
<?php
while ($plan = $db->fetch_assoc($result)) {
	if($plan['id'] != 0) {
		$plan['diskspace'] = mbgb($plan['diskspace']);
		$plan['bandwidth'] = mbgb($plan['bandwidth']);
?>
  <div id="plan<?=$plan['id']?>">
	  <span class="plans"><?=$plan['diskspace']?> disk space</span><br />
	  <span class="plans"><?=$plan['bandwidth']?> bandwidth</span><br />
	  <div class="price"><a href="/plans.php"><img src="<?=$pun_config['o_base_url']?><?=$plan['price_img']?>" alt="<?=$plan['plan_name']?>" /></a></div>
  </div>
<?php
	}
}
...
<?php
$tpl_temp = trim(ob_get_contents());
$tpl_main = str_replace('<spoon_plans>', $tpl_temp, $tpl_main);
ob_end_clean();
